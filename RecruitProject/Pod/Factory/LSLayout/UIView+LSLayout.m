...
//origin
-(void (^)(CGFloat, CGFloat))originBy
{
    return ^void (CGFloat x,CGFloat y){
        if (y>=0) {
            self.top = self.superview.top.offset(y);
        }
        
        if (x>=0) {
              self.left = self.superview.left.offset(x);
        }
...
-(void (^)(CGFloat, CGFloat, CGFloat, CGFloat))edgeNearTop
{
 return ^void (CGFloat top ,CGFloat left,CGFloat right ,CGFloat with){
     self.edge(top,left,-1,right);
     self.sizeBy(-1,with);
 };
}

-(void (^)(CGFloat, CGFloat, CGFloat, CGFloat))edgeNearbottom
{
    return ^void (CGFloat bottom ,CGFloat left,CGFloat right ,CGFloat with){
        self.edge(-1,left,bottom,right);
        self.sizeBy(-1,with);
    };
...
return ^void (UIView *view,NSLayoutAttribute attribute)
    {
        ...
        self.constraint(attribute).equalTo(view.constraint(attribute));
...
-(UIView *)sureSuperViewBy:(UIView *)view
{
    ...
    }else
    {
        ...
        //兄弟控件或其他族系
        UIView *superview1 = [self sureSuperView];
        UIView *superview2 = [view sureSuperView];
        ...
        do {
            ...
            do {
                ...
                superview1 = [superview1 sureSuperView];
                
            } while (superview1);
            
            superview2 = [superview2 sureSuperView];
